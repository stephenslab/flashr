...
plot_one_factor = function(f,pve,k,f_labels = NA,y_lab = "factor values"){
  ...
  if(any(is.na(f_labels))){
    ...
    plot_f = ggplot2::ggplot(f_dat,aes(x = variable,y = Factor,fill = sign.f )) +
    ...
    plot_f = ggplot2::ggplot(f_dat,aes(x = variable,y = Factor,label = variablenames,fill = sign.f)) +
      geom_bar(stat = "identity",width = 0.5) +
...
flash_plot_pve = function(f,main="Scree plot of PVE for each factor"){
  ...
  p <- ggplot2::ggplot(pve_dat, aes(factor_index, PVE)) + geom_point(size = 4)  +
    geom_line(linetype = "dotdash") + labs(title = main)
  return(p)
}
