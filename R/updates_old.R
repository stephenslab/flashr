# some old code i want to keep hanging around for now, eg for testing/comparisons
...
flash_optimize_single_fl_old = function(data, f, k, var_type, nullcheck = TRUE, tol = 0.01, ebnm_fn = ebnm_ash, ebnm_param = flash_default_ebnm_param(ebnm_fn),
    verbose = FALSE) {
...
flash_r1_old = function(data, f_init = NULL, var_type = c("by_column", "constant"), init_fn = "udv_si", tol = 0.01,
    ebnm_fn = ebnm_ash, ebnm_param = flash_default_ebnm_param(ebnm_fn), verbose = FALSE, nullcheck = TRUE) {
    if (is.matrix(data)) {
        data = flash_set_data(data)
    }
    var_type = match.arg(var_type)
    f = flash_add_factors_from_data(data, f_init = f_init, init_fn = init_fn, K = 1)
    f = flash_optimize_single_fl_old(data, f, flash_get_k(f), var_type, nullcheck, tol, ebnm_fn, ebnm_param, verbose)
    return(f)
}
