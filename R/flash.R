...
#' flash_get_sizes(f)
#' @export
flash_r1 = function(data,f_init=NULL,var_type = c("by_column","constant"), init_fn = "udv_si",tol=1e-2,ebnm_fn = ebnm_ash, ebnm_param=list(),verbose = FALSE, nullcheck=TRUE){
...
#' @export
flash_add_greedy = function(data,Kmax=1,f_init = NULL,var_type = c("by_column","constant"), init_fn="udv_si",tol=1e-2,ebnm_fn = ebnm_ash, ebnm_param=list(),verbose=FALSE,nullcheck=TRUE){
  if(is.matrix(data)){data = flash_set_data(data)}
  var_type=match.arg(var_type)
  f = f_init
...
#' flash_get_sizes(fb2)
#' @export
flash_backfit = function(data,f,kset=NULL,var_type = c("by_column","constant"),tol=1e-2,ebnm_fn = ebnm_ash, ebnm_param=list(),verbose=FALSE,nullcheck=TRUE,maxiter = 1000){
  if(is.matrix(data)){data = flash_set_data(data)}
