...
#' @details This is the main interface for fitting EBMF models based on algorithms from Wang and Stephens.
...
#' @param greedy a TRUE/FALSE flag to say whether to start by running the greedy algorithm (if FALSE then f_init must be supplied)
#' @param backfit a TRUE/FALSE flag to say whether to run backfitting
#' @seealso flash_add_greedy, flash_backfit
#' @return A fitted flash object. Use \code{flash_get_ldf} to access standardized loadings and factors; use \code{flash_get_lf} to access fitted LF'.
#' @examples
...
flash = function(data,
  ...
  if (backfit) {
    f = flash_backfit(data, f, kset=NULL, var_type, tol, ebnm_fn, ebnm_param, verbose, nullcheck)
  }
  return(f)
}
...
#' @details This implements the greedy algorithm from Wang and Stephens.
#' It can be used to adds factors to an existing fit, or start from scratch.
#' It adds factors iteratively, at each stage adding a new factor and then optimizing it.
...
flash_add_greedy = function(data,
  ...
  for (k in 1:Kmax) {
    ...
    f = flash_r1(data,
                 ...
                 nullcheck,
                 seed = NULL)
    #test whether the factor/loading combination is effectively 0
