test_that("flash object interface works as expected", {
  ...
  expect_equal(length(fo$history), 2)

  fo = flash_backfit(Y, fo)
  expect_s3_class(fo, "flash")
  expect_equal(length(fo$history), 3)
  ...
  expect_equal(length(fo$history), 4)
  ...
  expect_s3_class(fo, "flash")
  expect_equal(length(fo$history), 6) # nullcheck causes repeated backfit

  fo = flash_add_greedy(Y, 1, fo)
  expect_s3_class(fo, "flash")
  expect_equal(length(fo$history), 7)
  expect_equal(fo$history[[7]]$zeroed_out, 7)
